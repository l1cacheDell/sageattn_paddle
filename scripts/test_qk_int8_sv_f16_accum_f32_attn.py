...
import torch
import numpy as np
from sageattn_custom_ops import qk_int8_sv_f16_accum_f32_attn as qk_int8_sv_f16_accum_f32_attn_paddle
from phi_sageattention import qk_int8_sv_f16_accum_f32_attn as qk_int8_sv_f16_accum_f32_attn_torch
...
v_t = torch.empty(bsz, seq_len, head_num, head_dim, dtype=torch.float16).cuda()
o_t = torch.empty(bsz, seq_len, head_num, head_dim, dtype=torch.float16).cuda()
q_scale_t = torch.empty(bsz, head_num, (seq_len + BLKQ - 1) // BLKQ * (BLKQ // WARPQ) * 8, device=q_t.device, dtype=torch.float32)
k_scale_t = torch.empty(bsz, head_num, (seq_len + BLKK - 1) // BLKK * (BLKK // WARPK) * 4, device=k_t.device, dtype=torch.float32)

head_dim_og = q_t.size(-1)
sm_scale = head_dim_og**-0.5

lse_t = qk_int8_sv_f16_accum_f32_attn_torch(q_t, k_t, v_t, o_t, q_scale_t, k_scale_t, tensor_layout, is_casual, _qk_quant_gran, sm_scale, return_lse)
...
lse_p = qk_int8_sv_f16_accum_f32_attn_paddle(q_p, k_p, v_p, o_p, q_scale_p, k_scale_p, tensor_layout, is_casual, _qk_quant_gran, sm_scale, return_lse)
