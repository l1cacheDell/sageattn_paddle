...
#pragma unroll
  ...
  { 
    ...
    if (lse_idx < qo_len)
    {
      lse_lane_ptr[0] = (math::ptx_log2(d[fq][k]) + m[fq][k]);  // TODO: here has some bug.
    }
  }
...
// tensor_layout 0 for [B, N, H, D] (NHD, b, s, head, dim), 
//               1 for [B, H, N, D] (HND)
std::vector<paddle::Tensor>  qk_int8_sv_f16_accum_f16_attn_buf_fwd(paddle::Tensor& query,
                    paddle::Tensor& key,
                    paddle::Tensor& value,
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PADDLE_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto kernel_func = qk_int_sv_f16_attn_buffer_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, SADataType::kInt8, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, ComputeUnit::kTensorCore, 
            ...
            // using PD216bit = typename pd_cvt::PD16bitTrait<>::DataType;
            using PD16bitRe = typename pd_cvt::PD16bitReTrait<DTypeOut>::DataType;

            PD16bitRe* output_data = output.data<PD16bitRe>();  
...
// qk_int_sv_f16 impl
// the previous one stands for buffer
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, SADataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
