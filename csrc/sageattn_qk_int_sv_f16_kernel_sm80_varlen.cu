...
#pragma unroll
  ...
  half   *V_lane_base_ptr = V + 
                            cu_seqlen[batch_id] * stride_seq_v + 
...
#pragma unroll
  ...
  uint32_t O_load_idx_lane_base = bx * CTA_Q + 
                                  CTA_Q / num_warps * warp_id + 
...
std::vector<paddle::Tensor> qk_int8_sv_f16_accum_f16_attn_varlen_fwd(
                    paddle::Tensor& query,  // total_seqlen x num_heads x head_dim, int8
                    paddle::Tensor& key,    // total_seqlen x num_heads x head_dim, int8
                    paddle::Tensor& value,  // total_seqlen x num_heads x head_dim, fp16
                    paddle::Tensor& output, // total_seqlen x num_heads x head_dim, fp16
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PADDLE_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto kernel_func = qk_int_sv_f16_attn_varlen_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, SADataType::kInt8, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), half, false, DTypeOut, ComputeUnit::kTensorCore, 
...
std::vector<paddle::Tensor> qk_int8_sv_f16_accum_f16_attn_inst_buf_varlen_fwd(paddle::Tensor& query,
                    paddle::Tensor& key,
                    paddle::Tensor& value,
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PADDLE_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto kernel_func = qk_int_sv_f16_attn_varlen_kernel<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, SADataType::kInt8, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), float, true, DTypeOut, ComputeUnit::kTensorCore, 
                                                          mask_mode, RETURN_LSE, false>;
                                                    ...
                                                    bool return_lse)
{
  ...
  switch (pv_accum_dtype_const) {
    ...
    case paddle::DataType::FLOAT16: {
      v = v.cast(paddle::DataType::FLOAT16);
      qk_int8_sv_f16_accum_f16_attn_varlen_fwd(quant_results[0], quant_results[2], v, o, quant_results[1], quant_results[3], 
                                                cu_seqlen_q, max_seqlen_q, max_seqlen_k, 
                                                tensor_layout, _is_causal, _qk_quant_gran, sm_scale, _return_lse);
      break;
    }
    case paddle::DataType::UNDEFINED: {
      v = v.cast(paddle::DataType::FLOAT16);
      qk_int8_sv_f16_accum_f16_attn_inst_buf_varlen_fwd(quant_results[0], quant_results[2], v, o, quant_results[1], quant_results[3], 
                                                  cu_seqlen_q, max_seqlen_q, max_seqlen_k, 
                                                  tensor_layout, _is_causal, _qk_quant_gran, sm_scale, _return_lse);
      break;
    }
