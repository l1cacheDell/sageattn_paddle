...
def segmented_mean_kernel(
    ...
    # 固定output_ptr的基准地址
    output_base_ptr = output_ptr + batch_idx * output_stride_batch
    
    # 遍历head和dim维度
    for head_block in range(0, num_heads, BLOCK_SIZE_HEAD):
        head_idx = head_block + tl.arange(0, BLOCK_SIZE_HEAD)
        mask_head = head_idx < num_heads
        
        for dim_block in range(0, head_dim, BLOCK_SIZE_DIM):
            dim_idx = dim_block + tl.arange(0, BLOCK_SIZE_DIM)
            mask_dim = dim_idx < head_dim
            
            # 初始化累加器
            accumulator = tl.zeros((BLOCK_SIZE_HEAD, BLOCK_SIZE_DIM), dtype=tl.float16)
            
            # 遍历序列块
            for seq_block in range(0, seq_len, BLOCK_SIZE_SEQ):
                seq_offset = seq_block + tl.arange(0, BLOCK_SIZE_SEQ)
                mask_seq = seq_offset < seq_len
                global_seq = seq_start + seq_offset
                ...
                # 加载数据
                chunk = tl.load(input_ptr_block, 
                              mask=mask_seq[:, None, None] & mask_head[None, :, None] & mask_dim[None, None, :],
                              other=0.0)
                accumulator += tl.sum(chunk.to(tl.float16), axis=0)
            
            # 计算均值
            mean = accumulator / tl.maximum(seq_len, 1)
            ...
            # 存储结果
            tl.store(output_ptr_block, 
                    mean.to(input_ptr.dtype.element_ty), 
                    mask=mask_head[:, None] & mask_dim[None, :])
...
def test_segmented_mean():
    ...
    # Triton实现
    triton_output = segmented_mean_triton(
        input, cu_seqlen,
        BLOCK_SIZE_SEQ=BLOCK_SIZE_SEQ,
        BLOCK_SIZE_HEAD=BLOCK_SIZE_HEAD,
        BLOCK_SIZE_DIM=BLOCK_SIZE_DIM
